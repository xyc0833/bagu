# 1 c语言的编译过程包含哪些阶段？
C 语言从源代码到可执行程序，需要经过预处理、编译、汇编、链接四个阶段：

预处理阶段处理以 ＃ 开头的预处理指令。包括展开宏定义 (#defne) 、处理条件编译 (#ifdef) ) 、包含头文件 (#include) 。预处理器会把头文件的内容复制到源文件中, 把

宏替换成实际内容。处理完成后生成i文件, 这是纯C代码, 没有预处理指令了。
编译阶段把预处理后的C代码翻译成汇编代码。编译器会做语法检查、语义分析、代码优化等工作。如果代码有语法错误, 这个阶段会报错。编译完成后生成.§文件, 里面是汇编语言。

汇编阶段把汇编代码翻译成机器码。汇编器会把汇编指令转换成二进制的机器指令, 生成.0或, obj 的目标文件。目标文件已经是机器码了, 但还不能直接执行, 因为可能引用了其他文件的函数。

链接阶段把多个目标文件和库文件组合成一个可执行文件。链接器会解析函数调用、变量引用, 把所有代码和数据整合在一起, 生成最终的可执行文件 (如 exe 或 a.out) 。

# 2 C 语言中 include <> 和 include "" 有什么区别？

#include <> 和 #include "" 都是用来包含头文件的预处理指令，区别在于查找头文件的路径顺序不同。

#include <>（尖括号）编译器会在**系统标准**路径下查找头文件。这些路径通常是编译器安装时配置好的，包括标准库头文件所在的目录。比如 #include <stdio.h> 会在系统目录（如 /usr/include）中查找 stdio.h。这种方式用于包含系统提供的标准头文件。

#include ""（双引号）编译器会先在**当前源文件所在目录**查找，如果找不到再去系统标准路径查找。比如 #include "myheader.h" 会先在源文件的同级目录找，找不到才去系统目录找。这种方式用于包含自己编写的头文件。

# 3 C 语言中 const 和 define 有什么区别？各自的应用场景是什么？

const（C 语言关键字）const 是 C 语言的关键字，定义的是只读变量。例如 const int MAX = 100，这会在内存中分配空间存储 100 这个值，只是这个变量不能被修改。const 有类型检查，编译器会检查类型是否匹配；而且 const 有作用域，在函数内定义的 const 变量只在函数内可见。


#define（预处理指令）#define 是预处理指令，在预处理阶段进行简单的文本替换。例如 #define PI 3.14，编译器会把代码中所有的 PI 替换成 3.14，就像查找替换一样。#define 定义的不是变量，只是一个文本替换规则，没有类型检查，也不占用内存空间。

# 4 C 语言中 static 关键字有哪些作用？
C 语言中的 static 关键字有三种主要用法，分别用于修饰局部变量、全局变量和函数。

1 修饰局部变量 ：static 会改变变量的生命周期和存储位置。
普通局部变量存于栈中，函数结束后销毁；
static 局部变量存于全局数据区，程序启动时分配、程序结束时销毁；
且 static 局部变量只初始化一次，函数多次调用时操作的是同一个变量，常用于计数器场景。

2 修饰全局变量 ：static 会限制变量的作用域。
普通全局变量在整个程序中可见，其他文件可通过 extern 访问；
static 全局变量只在当前文件内可见，其他文件无法访问，实现了文件级封装，避免命名冲突。

3 修饰函数static 同样限制函数的作用域。
static 函数只能在当前文件内调用，相当于文件的私有函数；
在大型项目中，不同文件可以有同名的 static 函数，互不干扰。

# 5 C 语言中 extern 关键字的作用是什么？如何使用？

extern 关键字用于声明一个变量或函数是在其他文件中定义的，告诉编译器 “这个东西存在，但不在这个文件里，链接时会找到”。
多文件变量共享在多文件项目中，如果一个全局变量在 file1.c 中定义，file2.c 想使用它，就需要用 extern 声明。
注意：extern 只是声明，不是定义，不会分配内存。定义只能有一次（在 file1.c 中），但可以有多个声明（在需要使用的文件中）。

# 6 C 语言中 sizeof 是函数还是运算符？它在什么时候计算？

sizeof 是运算符，不是函数。虽然使用时写成 sizeof(int) 很像函数调用，但它本质上是**编译时的运算符**。
计算时机：sizeof 在编译阶段就计算出结果，而不是在程序运行时。编译器会根据类型信息直接计算出字节数，然后把 sizeof 表达式替换成一个常量。比如 sizeof(int) 在编译时就被替换成 4（假设 int 是 4 字节），运行时不需要任何计算，因此 sizeof 的性能开销为 0。

特殊特性：由于 sizeof 是编译时计算的，它不会真正执行表达式。比如 sizeof(i++) 不会让 i 增加，只是计算 i 的类型占多少字节。这是很多人容易犯的错误，误以为 sizeof 会执行里面的代码。

# 7 c语言中 大端 小端 


大端（Big Endian）和小端（Little Endian）是多字节数据在内存中的存储顺序，用来规定：
多字节数值（比如 int 占 4 字节）的高位字节和低位字节，谁放在低地址、谁放在高地址。
举个例子：数值 0x12345678（4 字节）
大端：高位字节存低地址 → 内存顺序：12 34 56 78
小端：低位字节存低地址 → 内存顺序：78 56 34 12
![alt text](image.png)

你这段代码用了 ** 共用体（union）** 来判断字节序，核心是利用 union 所有成员共享同一块内存：
```c
union {
    int i;   // 4字节
    char c;  // 1字节，指向同一块内存的起始地址（低地址）
} test;

test.i = 1;  // 给int赋值1，二进制：0x00000001
return (test.c == 0);
```
小端模式：1 的低位字节 0x01 存在低地址 → test.c 读到 1 → 返回 0（假）
大端模式：1 的高位字节 0x00 存在低地址 → test.c 读到 0 → 返回 1（真）
所以这个函数的含义是：
返回 1 → 当前是大端
返回 0 → 当前是小端

三、为什么要区分大端 / 小端？
大端：符合人类阅读习惯（高位在前），常用于网络传输、嵌入式设备。
小端：符合 CPU 运算习惯（低位先处理），x86、ARM 等主流 PC / 手机 CPU 默认小端。
网络传输时必须统一用大端（网络字节序），所以跨平台 / 网络编程时要做字节序转换（htons/ntohl 等函数）。

四、补充记忆口诀
大端：高字节存低地址 → “高对低”
小端：低字节存低地址 → “低对低”
代码判断：给 int 赋值 1，看首字节是不是 0 → 是则大端，否则小端。

# 8 C 语言中 volatile 关键字的作用是什么？在什么场景下使用？

volatile 关键字告诉编译器 “这个变量可能会被意外改变，不要对它做优化”，简单说就是防止编译器自作聪明。
编译器优化代码时，如果发现一个变量在一段代码中多次读取但没有写入，可能会把变量值缓存在寄存器中，后续直接读寄存器而不是从内存读。这种优化通常没问题，但有些特殊情况下，变量的值可能被外部因素改变（如硬件、中断、其他线程），如果还读缓存的旧值就错了。
volatile 就是告诉编译器 “这个变量可能随时变化，每次使用都要从内存重新读取，不要优化”。这样能确保读到的永远是最新值。

使用场景：
多线程编程：多个线程共享的变量可能被其他线程修改
中断服务程序：中断处理函数可能修改某些标志变量
硬件寄存器：映射到硬件寄存器的变量，值由硬件改变

# 9 C 语言的数据类型有哪些？各占多少字节？
C 语言的数据类型可以分为基本类型、构造类型、指针类型、空类型四大类。
1. 基本类型
整型：
char：1 字节（有无符号版本，字节数相同）
short：2 字节
int：4 字节
long：4 或 8 字节（取决于系统）
long long：8 字节
每种整型都有 unsigned（无符号）版本，表示范围不同但字节数一致。
浮点型：
float：4 字节
double：8 字节
long double：8 或 16 字节（取决于系统）
2. 构造类型
包括数组、结构体（struct）、联合体（union）、枚举（enum）。它们的大小取决于具体定义，不是固定值。
3. 指针类型
32 位系统：统一 4 字节
64 位系统：统一 8 字节
无论指向什么类型，所有指针大小一致；void* 是通用指针，大小与其他指针相同。
4. 空类型
void 类型不占字节，主要用于修饰函数返回值、参数或通用指针。

# 10 C 语言中什么是指针？指针和地址有什么区别？

指针是一个用来存储变量的地址 可以用它直接访问内存中的数据

地址：是内存中的一个位置，就像门牌号一样，每个内存单元都有唯一的地址。


# 11 C 语言中指针和数组有什么关系？数组名是指针吗？

数组名在大多数情况下会自动转换（退化）为指向数组首元素的指针。
例如 int arr[10]，arr 通常等价于 &arr[0]，都是指向第一个元素的指针。
这就是为什么可以用指针来遍历数组：*(arr + i) 等价于 arr[i]


数组名不是指针变量，它是一个常量指针，不能被赋值或修改：
arr = arr + 1 是非法的（数组名不可修改）。
若定义 int *p = arr，则 p = p + 1 是合法的（指针变量可以修改）。
可以把数组名理解成一个不可修改的指针常量。

# 12 C 语言中什么是野指针？如何避免野指针？

野指针（Wild Pointer）是指指向未知内存区域的指针，是 C 语言中最危险的问题之一。
使用野指针会导致程序崩溃（段错误）或产生不可预测的行为。

1 指针未初始化：定义后未赋值，内部是随机地址值。
```c
int *p;  // p 是随机地址
*p = 100;  // 危险！可能崩溃
```
2 内存释放后继续使用：指针指向的内存被 free() 释放，但指针仍被使用。
```c
int *p = (int *)malloc(sizeof(int));
free(p);
*p = 100;  // 危险！p 指向的内存已释放
```

3 指向栈上局部变量：函数返回后，栈上局部变量的内存失效，指针仍指向该区域。
```c
// 情况3：返回局部变量地址
int* get_local() {
    int x = 10;
    return &x;  // 危险！x 是局部变量，函数返回后内存无效
}
```
避免野指针的方法：
1 指针定义时立即初始化，没有明确地址就初始化为 NULL
2 free 后立即把指针设为 NULL，防止重复使用
3 不要返回局部变量的地址
4 使用前检查指针是否为 NULL


# 13 C 语言中什么是悬空指针？和野指针有什么区别？

悬空指针（Dangling Pointer）是指指向已经被释放或已经不存在的内存的指针。它和野指针类似，都指向无效内存，但产生原因稍有不同。

悬空指针的产生：
通常产生于内存释放后。当你 free 一块动态分配的内存，或者函数返回后栈上的局部变量被销毁，但指向这些内存的指针还存在，这个指针就成了悬空指针。它指向的地址曾经有效，但现在无效了。

与野指针的区别：
野指针的范围更广，包括所有指向未知或无效内存的指针。未初始化的指针是野指针，悬空指针也是一种野指针。可以说悬空指针是野指针的一个子集，特指那些曾经有效、现在无效的指针。

# 14 C 语言中 malloc、calloc 和 realloc 有什么区别？

malloc、calloc 和 realloc 都是动态内存分配函数，但使用方式和特点不同。

malloc（memory allocation） ： 配合sizeof使用
最基础的内存分配函数，分配指定字节数的内存。它只管分配，不初始化，分配的内存里是随机值（垃圾数据）。使用时要指定字节数，通常用 sizeof 计算，比如 malloc(sizeof(int) * 10) 分配 10 个 int 的空间。

calloc（contiguous allocation）分配内存并初始化为 0。
它接受两个参数：元素个数和每个元素的字节数。比如 calloc(10, sizeof(int)) 分配 10 个 int 并全部初始化为 0。如果需要干净的内存，用 calloc 更方便，不用手动 memset。

realloc（re-allocation）
用于调整已分配内存的大小。它接受一个旧指针和新的大小，会尝试扩展或缩小内存。如果原位置后面有足够空间，就地扩展。如果没有，会分配新内存，复制数据，释放旧内存。realloc 可能返回新地址，所以要用返回值更新指针。

# 15 C 语言中什么是内存泄漏？如何检测和避免内存泄漏？

内存泄漏（Memory Leak）是指程序动态分配的内存没有被释放，导致内存无法回收，可用内存越来越少。长时间运行的程序如果有内存泄漏，最终会耗尽系统内存，导致程序崩溃或系统变慢。

常见原因
malloc/calloc 分配的内存忘记 free；
指针被覆盖，原来指向的内存无法释放；
条件分支中只有部分路径释放内存；
异常退出导致 free 没有执行。

避免内存泄漏的方法
每次 malloc 后，都要在对应的位置加上 free
函数有多个返回路径时，确保每条路径都释放了内存
使用工具检测（Valgrind、AddressSanitizer）
用智能指针或内存池等高级技术

# 16 C 语言中栈和堆有什么区别？各自的特点是什么？

栈（Stack）和堆（Heap）是程序运行时内存的两个重要区域，它们的管理方式和特点完全不同。

一、栈（Stack）
用途：存储局部变量、函数参数、返回地址等。
管理方式：由系统自动管理，函数调用时分配，函数返回时自动释放。
生长方向：通常从高地址向低地址（向下生长）。
大小限制：通常有限制，Linux 默认是 8MB，超过会导致栈溢出（Stack Overflow）。
特点：速度快（仅需移动栈顶指针），但大小受限，生命周期固定。

二、堆（Heap）
用途：用于动态内存分配，通过 malloc、calloc、realloc 等函数申请。
管理方式：由程序员手动管理，malloc 后必须 free，否则会造成内存泄漏。
生长方向：通常从低地址向高地址（向上生长）。
大小限制：只受系统内存限制，远大于栈。
特点：灵活（可在运行时按需分配），但速度相对较慢，需要手动管理。

# 17 C 语言中函数指针是什么？如何定义和使用函数指针？

函数指针是指向函数的指针，它存储的是函数的入口地址。通过函数指针可以间接调用函数，实现回调、动态调用等高级功能。
1. 定义语法
函数指针的定义语法比较特殊：返回类型 (*指针名)(参数列表)。例如：
int (*func_ptr)(int, int);

2. 使用方法
赋值：让指针指向一个函数（函数名就是函数地址）：
int add(int a, int b) { return a + b; }
func_ptr = add;  // 函数名直接赋值给函数指针
调用：可以用两种写法，效果完全相同：
int res1 = (*func_ptr)(1, 2);  // 显式解引用调用
int res2 = func_ptr(1, 2);     // 简化写法，C语言允许直接用指针名调用

# 18 C 语言中指针的指针（二级指针）有什么用？如何理解？

二级指针（指针的指针）是指向指针的指针，用 ** 表示。如果说普通指针存的是变量的地址，那二级指针存的就是指针变量的地址。

1. 如何理解二级指针
理解二级指针的关键是分层思考：
int 类型的变量存整数；
int* 类型的指针存 int 变量的地址；
int** 类型的指针存 int* 指针的地址；
每多一个 *，就多一层间接访问。

2. 二级指针的典型用途
二级指针的核心作用是修改指针本身的值。
C 语言是值传递，传递一级指针只能修改指针指向的内容，不能修改指针本身；
若函数需要修改一个指针，让它指向新地址，就必须传递二级指针；
在函数内部修改 *pp，就能改变原来的一级指针 p 的指向。

# 19 c语言中const修饰指针有哪几种情况？ 分别是什么含义？ 

一、const 原本的含义
const 是 constant（常量） 的缩写，在 C 语言中用来修饰变量，表示该变量的值不可被修改（只读）。
被 const 修饰的变量必须在定义时初始化，之后无法再赋值。
本质是告诉编译器：“这个变量的值不应该被改变，禁止对它进行写操作”。

const 修饰指针时，会根据 const 相对于 * 的位置，分别限制指针本身或指针指向的内容，
这是 C 语言面试的高频考点。
情况 1：指向常量的指针（const 在 * 左边）
const int *p;
// 或等价写法
int const *p;

含义：指针指向的内容不能通过这个指针修改（即 *p 是只读的），但指针本身可以改变指向（即 p 可以指向其他地址）。
示例：*p = 100; 会报错，但 p = &other_var; 是合法的。

情况 2：指针常量（const 在 * 右边）
含义：指针本身不能改变指向（即 p 是只读的），但可以通过指针修改指向的内容（即 *p 可以被修改）。
示例：p = &other_var; 会报错，但 *p = 100; 是合法的。

情况 3：指向常量的指针常量（两个 const）
const int * const p;
含义：指针不能改变指向，也不能通过指针修改指向的内容，指针本身和指向的内容都是只读的。
示例：p = &other_var; 和 *p = 100; 都会报错。


# 20 C 语言程序的内存布局是怎样的？内存分区有哪些？
C 语言程序在内存中被分成几个不同的区域，每个区域存储不同类型的数据，
由低地址到高地址依次是：代码区、常量区、全局 / 静态区、堆区、栈区。
1. 代码区（Text Segment）
存储程序的机器指令，即编译后的可执行代码。
通常是只读区域，防止程序被意外修改。
程序加载时确定，运行期间地址和内容不变。
2. 常量区
存储字符串常量和其他常量数据。
例如 char *str = "hello" 中的 "hello" 就存放在常量区，不可被修改，尝试修改会导致段错误。
3. 全局 / 静态区
存储全局变量和 static 修饰的变量，分为两个子区域：
.data 段：存放已初始化的全局 / 静态变量。
.bss 段：存放未初始化的全局 / 静态变量，会被自动初始化为 0。
程序启动时分配，程序结束时释放。
4. 堆区（Heap）
用于动态内存分配，由 malloc/calloc/realloc 等函数申请。
生长方向：从低地址向高地址生长。
大小几乎只受系统总内存限制。
5. 栈区（Stack）
存储局部变量、函数参数、返回地址等。
生长方向：从高地址向低地址生长。
每次函数调用会分配一个栈帧（Stack Frame），函数返回时自动释放。
大小有限，通常为 8MB，超过会发生栈溢出。
内存布局地址顺序（从低到高）
代码区 → 常量区 → 全局/静态区 → 堆区 → 栈区

# 21  C 语言中结构体内存对齐是什么？为什么需要内存对齐？

内存对齐（Memory Alignment）是指数据在内存中的起始地址必须是某个数的倍数。例如 4 字节的 int 通常要求地址是 4 的倍数，8 字节的 double 要求地址是 8 的倍数。

一、结构体内存对齐的三个规则
起始地址对齐：结构体的起始地址必须是最宽成员大小的倍数。
成员偏移对齐：每个成员相对于结构体起始地址的偏移量必须是该成员自身大小的倍数，不足时填充字节补齐。
总大小对齐：结构体的总大小必须是最宽成员大小的倍数，不足时在末尾填充字节补齐。

二、为什么需要内存对齐？
主要是硬件效率的要求：
很多 CPU 一次只能从对齐的地址读取数据，如果数据不对齐，要么直接报错，要么需要多次内存访问才能读取完整数据，导致性能下降。
对齐后的数据，CPU 一次访问就能完成读取，速度大幅提升。
虽然对齐会浪费少量内存（填充字节），但换来的性能提升是值得的。

# 22 如何用 C 语言实现面向对象编程？

虽然 C 语言是面向过程的语言，但可以通过结构体、函数指针等机制模拟面向对象的三大特性：封装、继承和多态。

1. 封装
用结构体存储数据，用函数操作数据，函数接受结构体指针作为参数（类似 C++ 的 this 指针）。
通过命名约定（如 类名_方法名）表达所属关系，配合头文件隐藏内部实现，只暴露接口。
```c
// 定义"类"
typedef struct {
    int x;
    int y;
} Point;

// "成员函数"
void Point_init(Point *p, int x, int y) {
    p->x = x;
    p->y = y;
}

void Point_print(Point *p) {
    printf("(%d, %d)\n", p->x, p->y);
}

// 使用
Point p;
Point_init(&p, 10, 20);
Point_print(&p);
```

2. 继承
通过结构体嵌套实现：在 “子类” 结构体的第一个成员位置放入 “父类” 结构体，这样子类地址与父类成员地址相同，可以安全地进行类型转换，访问父类成员时直接访问子类的第一个成员即可
```c
#include <stdio.h>
#include <stdlib.h>

// 1. 定义「父类」：基础图形（Shape）
typedef struct {
    int x;  // 图形左上角x坐标
    int y;  // 图形左上角y坐标
    // 父类方法：移动图形
    void (*move)(void *self, int dx, int dy);
} Shape;

// 父类方法实现：移动图形
void Shape_move(void *self, int dx, int dy) {
    // 把通用指针转换为父类指针，访问父类成员
    Shape *shape = (Shape *)self;
    shape->x += dx;
    shape->y += dy;
    printf("图形移动到：(%d, %d)\n", shape->x, shape->y);
}

// 父类初始化函数（类似构造函数）
void Shape_init(Shape *shape, int x, int y) {
    shape->x = x;
    shape->y = y;
    shape->move = Shape_move; // 绑定父类方法
}

// 2. 定义「子类」：圆形（Circle），继承自Shape
// 核心：子类第一个成员必须是父类结构体，保证地址对齐
typedef struct {
    Shape super;  // 父类成员（继承核心）
    int radius;   // 子类独有成员：半径
    // 子类方法：打印圆形信息
    void (*print)(void *self);
} Circle;

// 子类方法实现：打印圆形信息
void Circle_print(void *self) {
    // 把通用指针转换为子类指针
    Circle *circle = (Circle *)self;
    // 通过子类的super成员访问父类属性
    printf("圆形 - 坐标：(%d, %d)，半径：%d\n", 
           circle->super.x, circle->super.y, circle->radius);
}

// 子类初始化函数（类似子类构造函数）
void Circle_init(Circle *circle, int x, int y, int radius) {
    // 先初始化父类（调用父类构造函数）
    Shape_init(&circle->super, x, y);
    // 再初始化子类独有属性
    circle->radius = radius;
    circle->print = Circle_print; // 绑定子类方法
}

int main() {
    // 3. 使用子类
    Circle *circle = (Circle *)malloc(sizeof(Circle));
    Circle_init(circle, 10, 20, 5); // 初始化子类

    // 调用子类方法
    circle->print(circle); // 输出：圆形 - 坐标：(10, 20)，半径：5

    // 核心：子类指针可以安全转换为父类指针，调用父类方法
    Shape *shape = (Shape *)circle; // 继承的关键：地址对齐，转换安全
    shape->move(shape, 3, 4);       // 调用父类move方法，输出：图形移动到：(13, 24)

    // 再次调用子类方法，验证父类属性已被修改
    circle->print(circle); // 输出：圆形 - 坐标：(13, 24)，半径：5

    free(circle);
    return 0;
}
```


3. 多态
通过函数指针实现：在结构体中定义函数指针成员（类似虚函数表），不同 “子类” 让函数指针指向不同的实现，调用时通过函数指针间接调用，即可实现多态。
```c
// "基类"
typedef struct {
    void (*draw)(); // 虚函数
} Shape;

// "子类1"
void draw_circle() { printf("画圆\n"); }
Shape circle = { draw_circle };

// "子类2"
void draw_rectangle() { printf("画矩形\n"); }
Shape rectangle = { draw_rectangle };

// 多态调用
Shape *shapes[] = { &circle, &rectangle };
for (int i = 0; i < 2; i++) {
    shapes[i]->draw(); // 不同对象调用不同实现
}
```
# 23 C 语言中宏定义和内联函数有什么区别？各有什么优缺点？

内联函数（inline function）是 C99 引入的特性，用 inline 关键字修饰。它的核心思想是：建议编译器在调用处直接展开函数代码，避免函数调用的栈操作开销，同时保留函数的语法结构、类型检查和作用域规则，比宏定义更安全、更易调试。

宏定义和内联函数都是为了避免函数调用开销，但实现机制完全不同。
1. 宏定义
实现：在预处理阶段做文本替换，不进行类型检查，没有函数调用开销。
#define MAX(a, b) ((a) > (b) ? (a) : (b))
会在预处理时直接替换成三元表达式
缺点：
缺少类型安全，参数无类型检查；
可能有副作用（如多次计算参数）；
调试困难，展开后的代码不直观。

2. 内联函数
实现：用 inline 关键字修饰，编译器会尝试在调用处展开函数代码，避免函数调用开销，但保留了函数的语义和类型检查。
注意：inline 只是建议，编译器可能不真正内联（如函数太复杂），但能保证代码正确性。

# 24 C 语言中如何实现可变参数函数？原理是什么？
可变参数函数是指参数个数不固定的函数，如 printf、scanf。C 语言通过 stdarg.h 中定义的宏来实现可变参数：va_list、va_start、va_arg、va_end。
1. 实现步骤
函数需要至少一个固定参数（通常用于指示可变参数的数量或格式），然后用省略号 ... 表示可变参数部分。
定义 va_list 类型的变量来遍历参数。
用 va_start 初始化可变参数列表，关联到固定参数。
用 va_arg 逐个获取参数，需指定参数类型。
用 va_end 清理可变参数列表，避免资源泄漏。
2. 原理
基于函数调用栈的连续性：
固定参数和可变参数会按顺序压入函数调用栈中。
通过固定参数的地址和参数类型的大小，可以计算出后续可变参数在栈中的地址。
va_arg 会根据指定的类型大小移动指针，从而依次访问每个可变参数。

```c
#include <stdarg.h>
#include <stdio.h>

// 计算多个整数的和
int sum(int count, ...) {
    int total = 0;
    va_list args;

    va_start(args, count);  // 初始化，count 是最后一个固定参数

    for (int i = 0; i < count; i++) {
        int value = va_arg(args, int);  // 获取下一个 int 参数
        total += value;
    }

    va_end(args);  // 清理

    return total;
}

int main() {
    printf("%d\n", sum(3, 10, 20, 30));   // 输出: 60
    printf("%d\n", sum(5, 1, 2, 3, 4, 5)); // 输出: 15
    return 0;
}
```

# 25 C 语言中递归和迭代有什么区别？什么时候用递归？
递归（Recursion）是函数调用自己，迭代（Iteration）是用循环重复执行。两者都能解决重复性问题，但实现方式和性能特点不同。
一、核心区别
递归：
核心是把大问题分解成小问题，直到问题简单到可以直接解决（递归终止条件），然后逐层返回结果。
代码通常更简洁优雅，特别适合处理树、图等递归结构，是最自然的表达方式。
缺点：每次调用都要在栈上分配栈帧，深度递归会消耗大量栈空间，可能导致栈溢出；且有函数调用开销，性能不如迭代。

迭代：
用循环实现重复，没有函数调用开销，不会消耗额外栈空间。
性能通常优于递归，也不会出现栈溢出。
缺点：代码可能不如递归直观，处理复杂递归结构时，需要手动维护栈或队列，代码会更复杂。
```c
// 递归实现
int factorial_recursive(int n) {
    if (n <= 1) return 1;  // 终止条件
    return n * factorial_recursive(n - 1);
}

// 迭代实现
int factorial_iterative(int n) {
    int result = 1;
    for (int i = 2; i <= n; i++) {
        result *= i;
    }
    return result;
}
```
二、什么时候用递归？
问题本身具有递归结构（如树、图的遍历、分治算法、回溯问题），用递归表达更自然、代码更简洁。
问题规模不大，递归深度可控，不会导致栈溢出。
可读性优先于极致性能的场景。

# 26 C 语言中如何避免头文件被重复包含？有哪些方法？

头文件被重复包含会导致重复定义错误，C 语言提供了两种方法来避免：条件编译和 #pragma once。

1. 条件编译（Include Guard）
这是传统方法，使用 #ifndef、#define、#endif 三个预处理指令组合。
原理：第一次包含时定义一个宏，再次包含时检测到宏已存在，就跳过整个文件内容。
优点：最可移植，所有编译器都支持。
```c
// myheader.h
#ifndef MYHEADER_H  // 如果没有定义过 MYHEADER_H
#define MYHEADER_H  // 就定义它

// 头文件内容
void my_function();

#endif  // 结束条件编译
```

2. #pragma once
这是现代方法，直接告诉编译器这个文件只包含一次。
写法更简单，但 #pragma 是编译器相关指令，理论上不是标准 C 的一部分。
优点：现代编译器（GCC、Clang、MSVC）都支持，性能通常比条件编译略好。
```c
// myheader.h
#pragma once

// 头文件内容
void my_function();
```

重复包含导致的问题示例
如果头文件没有保护，重复包含会引发编译错误：
```c
// header.h
struct Point {
    int x;
    int y;
};

// main.c
#include "header.h"
#include "header.h"  // 重复包含
// 编译错误: struct Point 重复定义
```